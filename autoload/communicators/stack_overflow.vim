function! g:communicators#stack_overflow#new()
  ...
  function! obj.fetch_stories(file_path)
    ...
    let headers = '-H "X-API-Access-Token: $WHATUPDOC_STACK_OVERFLOW_ACCESS_TOKEN" '
    ...
    let team = '&team=stackoverflow.com%2fc%2f$WHATUPDOC_STACK_OVERFLOW_TEAM'
    let key = '&key=$WHATUPDOC_STACK_OVERFLOW_KEY"'
    let unzip = ' | gunzip '
  ...
  function! obj.normalize_response(response, file_path, link_parsers)
      ...
      if has_key(response, 'items')
        for item in response.items
          ...
          if len(uniq_lines) > 0
            call add(normalized_stories, {'name':item.title, 'url':item.link, 'lines': uniq_lines})
          endif
        endfor
      endif
